...
{n:2,d:"2026-09-01",t:"Paradigmas de la utilización de medios",u:1,desc:"Paradigmas técnico, práctico y estratégico."},
...
{n:6,d:"2026-09-21",t:"Mirada crítica sobre la educación",u:2,desc:"Tecnologías en y para la educación. Rol e identidad docente en la era digital."},
{n:7,d:"2026-09-28",t:"Tecnología educativa y gamificación",u:2,desc:"Tecnología educativa en sentido didáctico. Gamificación."},
...
const unidades={1:["Conceptos y enfoques sobre medios","Clases 1 · 2 · 3"],2:["Cultura, crítica y tecnología educativa","Clases 4 · 5 · 6 · 7"],3:["Inclusión y accesibilidad educativa","Clases 8 · 9 · 10 · 11 · 12"]};
function disponible(d){return new Date()>=new Date(d+"T00:00:00")}
function bonito(d){const [y,m,x]=d.split("-");return `${x}/${m}`}
function render(){const root=document.getElementById("roadmap");root.innerHTML="";[1,2,3].forEach(u=>{let s=document.createElement("section");s.id=`unidad-${u}`;s.className=`unit unit-${u}`;s.innerHTML=`<div class="unit-head"><div class="number">UNIDAD ${u}</div><h2>${unidades[u][0]}</h2><p>${unidades[u][1]}</p></div><div class="cards"></div>`;let cards=s.querySelector(".cards");clases.filter(c=>c.u===u).forEach(c=>{let ok=disponible(c.d),card=document.createElement("article");card.className=`card ${ok?"available":"future-card"}`;card.innerHTML=`<div class="card-top"><div class="num">${String(c.n).padStart(2,"0")}</div><div class="date">${bonito(c.d)} · 2026</div></div><h3>${c.t}</h3><p>${c.desc}</p><div class="status ${ok?"available":"future"}">${ok?`● DISPONIBLE<br><a class="open-btn" href="clases/clase${String(c.n).padStart(2,"0")}.html">INGRESAR →</a>`:`🔒 PRÓXIMAMENTE · SE HABILITA EL ${bonito(c.d)}`}</div>`;cards.appendChild(card)});root.appendChild(s)});let n=clases.filter(c=>disponible(c.d)).length;document.getElementById("progress-label").textContent=`${n} de 12 clases disponibles`;document.getElementById("progress-bar").style.width=`${n/12*100}%`}render();